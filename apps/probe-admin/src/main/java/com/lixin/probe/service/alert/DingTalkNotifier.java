    ...
    public void send(AlertChannel channel, ChangeAlertRecord alert) {
        try {
            ...
            if (webhookUrl == null || webhookUrl.isEmpty()) {
                log.warn("[DingTalk] No webhookUrl configured for channel {}", channel.getName());
                return;
            }
            ...
            log.info("[DingTalk] Notification sent: status={}", response.statusCode());
        } catch (Exception e) {
            log.error("[DingTalk] Failed to send notification: {}", e.getMessage());
        }
    }
}
